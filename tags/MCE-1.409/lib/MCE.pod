...
This document describes MCE version 1.409
...
Below, a new instance is configured with all available options.
   ...
   my $mce = MCE->new(
      ...
      user_args    => { env => 'test' }, ## Default is undef
...
Note: As mentioned above, sequence can be specified using an array
reference instead: sequence => [ 10, 19, 0.7, "4.1f" ]
