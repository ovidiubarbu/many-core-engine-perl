...
This document describes MCE version 1.400
...
=head2 new ( [ options ] )
   ...
   my $mce = MCE->new(
      ...
      input_data   => $input_file,       ## Default is undef
          ...
          ## Starting with MCE release 1.4, one can specify the
      ...
      sequence     => {                  ## Default is undef
          ...
          ## Beginning with MCE release 1.4, sequence can be specified
          ## using an array reference (3rd & 4th values are optional):
          ##
...
=head2 SEQUENCE
   ...
   ## Note: MCE release 1.4 allows sequences to be specified using an
   ## ARRAY reference instead: sequence => [ 10, 19, 0.7, "4.1f" ]
   ## The 3rd & 4th array values are optional.
...
=head2 send ( $data_ref )
   ...
   ## to each available worker before running. One cannot send more
   ## than the total workers spawned. The next available worker,
   ## without data, stores the received data as $self->{user_data}.
   ##
   ## The data which can be sent is restricted to an ARRAY, HASH, or PDL
   ## reference. The send method cannot be called once MCE is running.
   ## After running, workers set $self->{user_data} back to undef.
   ##
   ## Note: When specifying the user_tasks option, the send method is
   ## applicable only to workers configured under the first task.
