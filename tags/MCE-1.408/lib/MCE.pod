...
This document describes MCE version 1.407
...
=head2 SEQUENCE
   ...
   ## The 1.3 release allows workers to loop through a sequence of
   ...
   ## Note: As mentioned above, sequence can be specified using an
   ## array reference instead: sequence => [ 10, 19, 0.7, "4.1f" ]
...
=head2 sess_dir ( void )
   ...
   ## Returns the session directory used by the MCE instance.
   ## This is defined during spawning and removed during shutdown.
...
=head2 forseq ( $sequence_spec [, { options } ], sub { ... } )
   ...
   ## Sequence can be specified using a hash or array reference.
...
=head2 send ( $data_ref )
   ...
   ## minimize memory consumption and afterwards send data individually
   ## to each worker. One cannot send more than the total workers
   ## spawned. Workers store the received data as $self->{user_data}.
   ##
   ## The data which can be sent is restricted to an ARRAY, HASH, or PDL
   ## reference. Workers begin processing immediately after receiving
   ## data. Workers set $self->{user_data} to undef after processing.
   ##
...
=head2 status ( void )
   ...
   ## The greatest exit status is saved among workers exiting during a
   ## run. Call this after runnning or processing to check if a worker
   ## exited with a non-zero value. For more granularity, look at the
...
Barrier synchronization (sync) was added to MCE 1.406.
   ...
   ## Barrier synchronization is supported for task 0 only.
   ## Workers are assigned task_id 0 when ommitting user_tasks.
   ## All workers must call sync whenever barrier syncing.
