...
This document describes MCE version 1.301
...
=head2 new ( [ options ] )
   ...
   my $mce = MCE->new(
      ...
      input_data   => $input_file,       ## Default is undef
          ...
          ## through a sequence of numbers in parallel.
      ...
      user_tasks   => [                  ## Default is undef
         ...
         { ... },                        ## Options for task 2
          ...
          ## Takes a list of hash references, each allowing for 6
          ## options: max_workers, sequence, user_begin, user_func,
          ## user_end, and task_end. Other options specified here
          ## are ignored. Input_data, if specified from the main
          ## MCE instance, is only applicable to task 0.
...
=head2 SEQUENCE
   ...
   ## each user_task. Input_data is only applicable to the first task.

   ## See a demo at the end of this documentation applying sequences
   ## with the user_tasks option.
...
=head2 USER_TASKS
   ...
   ## The 1.200 release introduced a new option to allow for flexible
...
=head2 SYNTAX for ON_POST_RUN
   ...
   ## whereas on_post_run below is called after running. The reference
...
=head2 restart_worker ( $wid )
   ...
   my $mce = MCE->new(
      ...
      on_post_exit => sub {
         ...
         $self->restart_worker($e->{wid});  ## Passing same wid as arg.
      },
...
directly from the forseq.pl example. Think of the following demonstration
as having 3 mini-MCEs running simultaneously in parallel.
