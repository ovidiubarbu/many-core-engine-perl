...
This document describes MCE::Core version 1.504
...
Below, a new instance is configured with all available options.
   ...
   my $mce = MCE->new(
      ...
      RS           => "::\n\n",          ## Default undef
          ...
          ## input_data => '/path/to/file'  for file path
          ## input_data => \@array          for array
          ## input_data => \*FILE_HNDL      for file handle
          ## input_data => \$scalar         treated like a file
          ## input_data => sub { }          for input iterator
          ...
          ## Release 1.505 adds support for input iterators
          ## via input_data => sub { }
...
MCE supports 5 ways to specify input_data. Support for input iterators made
its 1st appearance in MCE 1.505. The RS option allows one to specify the record
separator when processing a file. Btw, input_data is optional and does not
need to be configured is simply wanting many workers doing different things.
...
containing the raw data (files only) instead of an array reference.
   ...
   use_slurpio => 1,                ## passes a scalar ref
   RS          => "::\n\n",         ## record separator for file

The chunk_size value determines the chunking mode to use on files. Otherwise,
chunk_size is the number of elements for arrays. Less than or equal to 8192 is
the number of records for files. Greater than 8192 is the number of bytes to
read including (the rest) up to the next record separator.
...
The format for user_func is as follow.

   user_func => sub {
      my ($self, $chunk_ref, $chunk_id) = @_;

      ## for scalar reference (use_slurpio => 1)
      ## $_ is $chunk_ref

      print $$chunk_ref;

      ## for array reference
      ## $_ is $chunk_ref->[0] when chunk_size => 1
      ## $_ is $chunk_ref otherwise
...
Let's now go over the input iterator. A great example of this is simulating a
sequence of numbers specified using input_data instead of the sequence option.

When time permits, take a look at the included iterator.pl example, behaving
similarly to forseq.pl with ordered output. Benchmark the two examples and
notice the difference. The included sequence engine via the sequence option is
faster due to zero involvement from the manager process. With that said, the
below is mearly a demonstration for making a simple iterator factory using
closures.

   use MCE;

   ## Provides a sequence of numbers.
   ...
   -- Output   Note that output order is not guaranteed.
               Peek at iterator.pl if wanting ordered output.
...
A lazy array cannot be specified for input_data due to MCE wanting to know
up-front the size of the array. Thus, make an iterator factory like so.
Below, the code block for the lazy array returns undef when completed.
...
The following example demonstrates how one can retrieve a chunk from the array
per each request. In this scenario, undef is sent by the iterator block when
reaching the maximum index value.
   ...
   ## Iterator with chunking support for lazy arrays.
...
This option takes an array of tasks. Each task allows up to 15 MCE options.
Both input_data and RS can be defined inside the first task only or at the
top level, otherwise both are ignored.
