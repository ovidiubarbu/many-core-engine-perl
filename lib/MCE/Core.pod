...
This document describes MCE::Core version 1.522
...
Below, a new instance is configured with all available options.
   ...
   my $mce = MCE->new(
      ...
      chunk_size => 2000,                ## Default $MCE::CHUNK_SIZE
          ...
          ## above 64M will be set to 64M (the maximum).
      ...
      init_relay => 0,                   ## Default undef
          ...
          ## For specifying the initial relay value. Used with the
          ## relay method for receiving and passing on information.
      ...
      RS         => "\n>",               ## Default undef
          ...
          ## and file handles. Typically, the left side is what happens
          ## when specifying ($/ = "\n>") in Perl. MCE performs a couple
          ## steps for RS beginning with a newline character. It chops
          ## the characters following the newline and prepends them
          ## automatically to subsequent records.
          ##
          ## Record 1:  >seq1 ... \n>   (to)   >seq1 ... \n
          ## Record 2:  seq2  ... \n>          >seq2 ... \n
          ## Record 3:  seq3  ... \n>          >seq3 ... \n
...
example findnull.pl for another use case.
   ...
   my $mce = MCE->new(
      ...
      user_func => sub {
         ...
         ## relay counters to next worker
         ## $mce->relay( sub { $_->[0] += $acc1; $_->[1] += $acc2 } );
...
The relay method is for receiving and passing on information. Relay is enabled
by specifing the init_relay option which takes a hash or array reference, or a
scalar value. MCE configures each worker with a unique socket pair. Also see
(findnull.pl, cat.pl, and biofasta/fasta_aidx.pl) for other use cases.
...
Inside the code block, $_ points to the hash or array reference, or contains
the scalar value. Basically, one updates in place. Omitting the code block
(e.g. MCE::relay) will simply relay without any updates.

Below, relaying multiple values (HASH reference);
...
or multiple values (ARRAY reference);
...
=item ->relay_recv ( void )

The relay_recv method allows one to perform an exclusive action prior to
relaying. Below, the user_func is taken from the cat.pl example. Think of
obtaining exclusive locking in an orderly fashion. Relaying is chunk_id
driven (or task_wid when not processing input), thus orderly.
