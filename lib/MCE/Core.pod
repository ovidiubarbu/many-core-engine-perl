...
example findnull.pl for other use case.

   use MCE max_workers => 4;

   my $mce = MCE->new(
      init_relay => [ 0, 100 ],       ## two counters; initial values
...
The relay method is for receiving and passing on information. Specify the
initial value via the init_relay option. Each worker is configured with a
unique socket pair when init_relay is specified. See included examples
