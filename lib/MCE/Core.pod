...
This document describes MCE::Core version 1.521
...
Below, a new instance is configured with all available options.
   ...
   my $mce = MCE->new(
      ...
      chunk_size => 2000,                ## Default $MCE::CHUNK_SIZE
          ...
          ## Smaller than or equal to 8192 is number of records.
          ## Greater than 8192 is number of bytes. MCE reads until
          ## the end of record before calling user_func. A value
          ## above 24M will be set to 24M (the maximum).
      ...
      RS => "::\n\n",                    ## Default undef
          ...
          ## Specify the sequence option if simply wanting to loop
          ...
          ## file handles, and scalar references. Support for
          ## iterators was added in 1.505.

      use_slurpio => 1,                  ## Default 0
      parallel_io => 1,                  ## Default 0

          ## Whether to enable slurpio when reading files.
          ...
          ## blades and input coming from shared storage.
      ...
      interval => {
          ...
          ## interval => 0.005           ## Shorter; MCE 1.506+
...
Run calls spawn, submits job; workers call user_begin, user_func, and
user_end. Run shuts down workers afterwards. Call the spawn method early
when the need arises for large data structures within the main process
itself prior to running.
...
via exit (children), MCE->exit (children and threads), or die.
...
once per each worker during a run.
...
Input_data, RS, and use_slurpio can be defined inside the first task or
at the top level, otherwise ignored under other sub-tasks.
...
=item use_slurpio => 1
   ...
   $_ is a ref regardless of whether chunk_size is 1 or greater
...
same as $_, due to chunk_size greater than 1.
...
For persistent workers, better to do the following instead. Workers will
stay alive until told to shut down or during the exiting of the script.
Below is shown with overriding the default for max_workers.
   ...
   $mce->spawn;      ## Spawn early if required
...
sequence, or user_tasks for the MCE instance in order to receive user data
via the "send" method.
...
automatically. Workers persist after calling the process method. Call shutdown
...
spawning workers early is desired.
...
after running or processing to check if any worker exited with a non-zero value.
...
do & sendto. The manager process processes these serially. This utilizes
the Storable Perl module for passing data from a worker process to the
manager process. The callback function can optionally return a reply.
...
Worker exits from MCE. $id (optional) can be used to pass the primary key
or a string along with the message. Please look at the on_post_exit and
...
Worker leaves the chunking loop or user func immediately. Callable from inside
...
A use case is an app configured with 200 workers running on a 24 logical way
...
at some point, rises to 90+. In addition, IP_Tables reaches its contention
point causing the entire app to fail.

The above is solved by simply having workers yield among themselves in a
synchronized fashion. A delay of 0.006 seconds between intervals is all that's
needed. The load on the box hovers between 23 ~ 27 for the duration of the run.
The actual poll completes in under 17 minutes time. This is quite fast
considering the app polls 62,500,000 metrics combined. The math equates to
3,676,470 per minute or rather 61,275 per second from a single node.

   ## Both max_nodes and node_id are optional (default is 1).

   interval => {
      delay => 0.006, max_nodes => $max_nodes, node_id => $node_id
   }
