...
This document describes MCE::Examples version 1.521
...
=head1 INCLUDED WITH THE DISTRIBUTION
   ...
   cat.pl, egrep.pl, wc.pl
             Concatenation script, similar to the cat binary.
   ...
   files_flow.pl, files_mce.pl, files_thr.pl
             ...
             See the MCE::Queue synopsis for another variation.
   ...
   ping.pl   Perform ping test and report back failed IP(s) to standard
             output.
   ...
   seq_demo.pl
             Demonstration of the new sequence option appearing
             in MCE 1.3. Run with seq_demo.pl | sort

   sync.pl   Barrier sync demonstration.
   ...
   utf8.pl   Demonstration of MCE processing unicode input_data.
...
with various models. The L<MCE::Loop|MCE::Loop> model is used for the next
demonstration.
...
The L<MCE::Loop|MCE::Loop> model is shown next. Looping is required inside the
code block, just like above.
