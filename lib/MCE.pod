...
3 constants defined in MCE which are exportable. Using the constant names in
lieu of 0,1,2 makes it more legibel when accessing the argument array.
...
The user_func block above can be written using one less line beginning with
MCE 1.5. The $_ scalar below contains the input sequence number.
...
Release 1.5 and later supports the use of constants for @_ index position
and the $_ input scalar.

   use MCE EXPORT_CONST => 1, MAX_WORKERS => 'auto', CHUNK_SIZE => 30;

   my $mce = MCE->new;

   ## foreach below can be written as foreach ( @{ $_ } ) as well
   ## using the input $_ scalar coming from forchunk.

   $mce->forchunk(\@input_data, sub {
      foreach ( @{ $_[CHUNK_DATA] } ) {
...
One can write the above using less code beginning with MCE 1.5. $_ below
contains the actual input value. MCE is a chunking engine. Keep in mind that
$_ will point to a reference whenever chunk_size is greater than 1 or
use_slurpio is enabled for other use case.
...
Using the $_ scalar below is possible beginning with MCE 1.5.
