...
Below, a new instance is configured with all available options.
   ...
   my $mce = MCE->new(
      ...
      name         => 'string',          ## Default undef
          ...
          ## The name value is passed as the 3rd arg to task_end.
      ...
      user_tasks   => [                  ## Default undef
         ...
         { ... },                        ## Options for task 2
          ...
          ## Takes a list of hash references, each allowing up to 11
          ## options. All other MCE options are ignored. Input_data
          ## is applicable for the first task only.
          ##   name, max_workers, chunk_size, input_data, sequence,
...
This option takes an array of tasks. Each task allows for 11 MCE options.
...
arguments $task_id and $name (shown below).
...
L<http://code.google.com/p/many-core-engine-perl/wiki/MCE_Tasks>
   ...
   my $mce = MCE->new(
      ...
      task_end   => sub {
         my ($self, $task_id, $name) = @_;
         print "Task [$task_id -- $name] completed processing\n";
      },
...
Returns the name value specified via the name option when configuring MCE.
