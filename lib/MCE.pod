...
This document describes MCE version 1.300
...
=head2 new ( [ options ] )
   ...
   my $mce = MCE->new(
      ...
      input_data   => $input_file,       ## Default is undef
          ...
          ## through a sequence of numbers in parallel. Leave
          ## input_data undef when specifying the sequence option.
      ...
      chunk_size   => 2000,              ## Default is 1
          ...
          ## Chunk_size is set to 1 when specifying sequence or
          ## calling $self->foreach or $self->forseq methods.
...
=head2 SEQUENCE
   ...
   ## numbers in parallel. Sequence can also be specified per task
   ## when specifying user_tasks for even greater flexibility. Take
   ## a look at the seq_demo.pl example to see sequence applied with
   ## user_tasks.

   ## The sequence option takes a hash reference containing up to four
   ## key => value pairs. Both step and format are optional. The format
   ## string (without the %) is passed to sprintf behind the scene.
   ##    e.g. $seq_n_formated = sprintf("%4.1f", $seq_n);

   ## STEP, if omitted, defaults to 1 if BEGIN is smaller than END.
   ## Otherwise, -1 if BEGIN is greater than END.

   ## Note: Brackets below indicate optional attributes. Remove them if
   ## wanting to run this code snippet as is.
...
=head2 USER_TASKS
   ...
   ## allows for 6 options:
   ##   max_workers, use_threads, user_begin, user_func, user_end, and
   ##   sequence (new in 1.300)
   ##
...
=head2 SYNTAX for ON_POST_RUN
   ...
   ## The on_post_exit is immediately after a worker exits,
   ## whereas on_post_run is after running. The reference
...
=head2 chunk_size ( void )
   ...
   ## Returns the Chunk Size used by MCE.
...
=head2 forseq ( $sequence_spec [, { options } ], sub { ... } )
   ...
   ## Forseq always implies chunk_size => 1 (cannot be overwritten).
...
=head2 restart_worker ( $wid )
   ...
   ## Please use the same wid from the worker which has exited.
   ...
   my $mce = MCE->new(
      ...
      on_post_exit => sub {
         ...
         $self->restart_worker($e->{wid});   ## Pass same wid as arg.
      },
...
=head2 DEMO SEQUENCES AND USER_TASKS
...
example.
   ...
   task_id 1: seq_n 22: chunk_id 2: wid 4: task_wid 2
   task_id 1: seq_n 23: chunk_id 3: wid 3: task_wid 1
   task_id 1: seq_n 24: chunk_id 4: wid 4: task_wid 2
   task_id 1: seq_n 25: chunk_id 5: wid 3: task_wid 1
   task_id 1: seq_n 26: chunk_id 6: wid 4: task_wid 2
   task_id 1: seq_n 27: chunk_id 7: wid 3: task_wid 1
   task_id 1: seq_n 28: chunk_id 8: wid 4: task_wid 2
   task_id 1: seq_n 29: chunk_id 9: wid 3: task_wid 1
   task_id 2: seq_n 31: chunk_id 1: wid 5: task_wid 1
   task_id 2: seq_n 32: chunk_id 2: wid 6: task_wid 2
   task_id 2: seq_n 33: chunk_id 3: wid 5: task_wid 1
   task_id 2: seq_n 34: chunk_id 4: wid 6: task_wid 2
   task_id 2: seq_n 35: chunk_id 5: wid 5: task_wid 1
   task_id 2: seq_n 36: chunk_id 6: wid 6: task_wid 2
   task_id 2: seq_n 37: chunk_id 7: wid 5: task_wid 1
   task_id 2: seq_n 38: chunk_id 8: wid 6: task_wid 2
   task_id 2: seq_n 39: chunk_id 9: wid 5: task_wid 1
