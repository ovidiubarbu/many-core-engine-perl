...
This document describes MCE version 1.600
...
involvement from the manager process while running.
...
MCE 1.5 includes 6 models. The models take Many-Core Engine to a new level
for ease of use. Two options (chunk_size and max_workers) are configured
automatically as well as spawning and shutdown.
