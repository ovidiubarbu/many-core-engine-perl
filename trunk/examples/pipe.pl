...
###############################################################################
## ----------------------------------------------------------------------------
## Process STDIN or FILE through an external command in parallel.
##
## This is by no means a complete script, but rather a "how-to" for folks
## wanting to create their own parallel node script processing STDIN or
## FILE through an external command.
##
## For UNIX, the Perl interpretor does "not" sub-shell when calling the
## external command.
##
###############################################################################
...
NAME
   $prog_name -- process STDIN or FILE through an external command in parallel

SYNOPSIS
   $prog_name [mce_options] command [options] [FILE]

DESCRIPTION
   The $prog_name script processes either STDIN or FILE in parallel
   utilizing efficient IO under the UNIX environment. STDIN is read
   ...
   --lang=LOCALE
          Override locale e.g. C, en_US.UTF-8, en_US.ISO-8859-1
...
EXAMPLES
   The "command" receives data through STDIN irregardless of the MCE script
   reading from STDIN or FILE. Notice the map and reduce idiom for the
   wc -l examples below.

   Process STDIN (slower, requires interaction by the manager process).
   ...
   Process FILE (faster, zero involvement from the manager process).
...
###############################################################################
## ----------------------------------------------------------------------------
## Display an error and exit if "command" is not found.
##
###############################################################################
