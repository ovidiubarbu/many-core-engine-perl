...
many bytes to read. MCE appends (the rest) up to the next record separator.
   ...
   chunk_size  =>  1000,            ## Consists of 1000 records
   chunk_size  => 16384,            ## Approximate 16384 bytes for files
   chunk_size  => 50000,            ## Approximate 50000 bytes for files
...
and files. What about iterators? Ok, the next several examples all use an
iterator reference for input_data.
