...
This document describes MCE::Core version 1.520
...
=head3 SELF CHUNK CID
   ...
   ## MCE 1.5 allows any public method to be called directly.
...
sequence engine to compute the 'begin' and 'end' items only for the chunk
...
will stay alive until told to shut down or during the exiting of the script.
...
do & sendto. The main MCE thread will process these serially. This utilizes
the Storable Perl module for passing data from a worker process to the main
MCE thread. The callback function can optionally return a reply.
...
along with calling the 'do' method. A stall may occur for workers calling sync
the 2nd or 3rd time while other workers are sending results via 'do' or
'sendto'. It requires yet another semaphore lock file in MCE to solve
