...
Below, a new instance is configured with all available options.
   ...
   my $mce = MCE->new(
      ...
      RS_prepend => ">",                 ## Default undef
          ...
          ## MCE scripts must specify RS => "\n>", RS_prepend => ">"
          ## for slurping FASTA files. Specifying RS => "\n>" by itself
          ## means that chunks 2 and higher will not start with ">"
          ## which will not work. The option was added to MCE 1.523.
      ...
      user_tasks => [                    ## Default undef
         ...
         { ... },                        ## Options for task 2
          ...
          ## RS, RS_prepend, and use_slurpio are applicable for the
          ## first task only.
...
reference.
   ...
   RS_prepend  => ">",              ## prepend ">" for chunks 2 and higher
...
Input_data, RS, RS_prepend, and use_slurpio may be defined inside the
first task or at the top level, otherwise ignored under other sub-tasks.
...
Allowable options { key => value, ... } are:
   ...
   user_func user_error user_output use_slurpio RS RS_prepend
