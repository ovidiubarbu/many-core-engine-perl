...
sequencer engine to compute the begin and end items only for the chunk and
not the items in between (hence boundaries only). This option has no effect
when sequence is missing or chunk_size equals 1.

   use MCE;

   my $mce = MCE->new(
      sequence    => { begin => 1, end => 10 },
      chunk_size  => 5,
      bounds_only => 1,

      ## For sequence, the input scalar $_ points to $chunk_ref
      ## when chunk_size > 1, otherwise has $chunk_ref->[0].
      ##
      ## user_func => sub {
