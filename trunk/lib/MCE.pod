...
This document describes MCE version 1.410
...
The anonymous user_func above can be written using one less line. There
are 3 constants defined in MCE which are exportable. Using the constant
names in lieu of 0,1,2 makes it more legibel when accessing the argument
array.

The user_func receives either 1 or 3 arguments. Three arguments are sent
whenever processing input data (specifying input_data or sequence).

=head3 SELF CHUNK_DATA CHUNK_ID

   ## The following exports SELF => 0, CHUNK_DATA => 1, CHUNK_ID => 2
...
In addition to EXPORT_CONST, five additional arguments can be passed to
MCE. This allows one to override the default configured in MCE.pm.
...
Below, a new instance is configured with all available options.
   ...
   my $mce = MCE->new(
      ...
      max_workers  => 8,                 ## Default $MCE::MAX_WORKERS
          ...
          ## Number of workers to spawn. Can be set automatically as well
          ## with MCE 1.5 and later releases.
      ...
      thaw         => \&decode_sereal,   ## Default $MCE::THAW
          ...
          ## Release 1.5 adds 2 new options to allow freeze/thaw to be
          ## overridden. Include a serialization module prior to loading
          ## MCE.
          ## e.g. use Sereal qw(encode_sereal decode_sereal);
...
whenever the need arises for large data structures within the main
process prior to running.
...
The 1.3 release and above allow workers to loop through a sequence of
numbers in calculated mathematically without the overhead of an array.
Sequence can be specified separately per each user_task entry unlike
input_data. Input_data is applicable to the first task only.

See a demo at the end of this documentation applying sequences with
the user_tasks option including chunking.
...
The user_func block above can be written using one less line beginning
with MCE 1.5. $_ is localized and points to an array reference when
chunk_size is greater than 1 or a scalar reference when use_slurpio is
enabled. Otherwise, $_ contains the scalar e.g. $_ = $_[CHUNK_DATA].
   ...
   my $mce = MCE->new(
      ...
      user_func => sub {
         print $_, " from ", $_[SELF]->wid(), " id ", $_[CHUNK_ID], "\n";
      }
   );

=head2 USER_TASKS

The 1.2 release introduced the user_tasks option to allow for multiple
roles. This option takes an array of tasks. Each task allows for 7 sub
options: max_workers, use_threads, user_begin, user_func, user_end,
sequence, and chunk_size (the latter 2 coming from 1.3).

In addition, task_end can be specified for the manager process to run
when all workers for that role have completed processing.

Forking and threading can be inter-mixed among tasks except under
Cygwin. Input data (if defined) can be processed by the first task
only. The run method will remain running until all workers have
...
option, if defined, is executed immediately after a worker exits via
exit (children only), $self->exit (both children and threads), or die.
...
The user_begin and user_end options, if specified, behave similarly
to awk 'BEGIN { ... } { ... } END { ... }'. These are called once per
each worker during a run.
...
When processing input data, MCE can pass an array or rows or a slurped
...
Output coming from $self->sendto('stderr/stdout', ...) can be intercepted
by specifying the user_error and user_output options. MCE on receiving
output will instead direct to user_error and user_output in a serialized
fashion. Handy when wanting to filter, modify, and/or direct the output
elsewhere.
...
is set to 0 for the manager process.
...
automatically spawned, the code block is executed in parallel, and
shutdown is called. Do not use these methods if wanting workers to
persist afterwards.
...
Foreach implies chunk_size => 1 and cannot be overwritten. Arguments
inside the code block are the same as for user_func. This holds true
even if chunk_size is set to 1. MCE is both a chunking engine plus a
parallel engine all in one. Arguments within the block are the same
whether calling foreach or forchunk.
...
One can write the above using less code beginning with MCE 1.5. In order
to not break the API for older releases, $_[CHUNK_DATA] is the same as
$chunk_ref above. $_ below contains the actual value due to chunk_size
being 1 for the foreach method.

MCE is a chunking engine. Keep in mind that $_ will point to a reference
whenever chunk_size is greater than 1 or use_slurpio is enabled for other
use case.

   use MCE EXPORT_CONST => 1, MAX_WORKERS => $max_workers;

   my $mce = MCE->new;

   $mce->foreach(\@input_data, sub {
      $_[SELF]->sendto("stdout", "$_[CHUNK_ID]: $_\n");
   });

Below, passing an anonymous array as input data. For example, wanting
to parallelize a serial loop with MCE.
...
Forseq also implies chunk_size => 1 and cannot be overwritten. Sequence
can be specified using a hash or an array reference.
...
The last block above can be written as follow beginning with MCE 1.5.

   $mce->forseq([ 20, 40 ], sub {
      my $result = `ping 192.168.1.${$_}`;
      ...
   });
...
due to restarting each time. The same wid from the worker, which has
exited, is used (recommended).
...
down workers. Set $auto_shutdown to 0 when wanting workers to persist
after running (default is 1).
...
memory consumption and afterwards send data individually to each worker.
One cannot send more than the total workers spawned. Workers store the
received data as $self->{user_data}.
...
Workers set $self->{user_data} to undef after processing.

One cannot specify input_data, sequence, or user_tasks for the MCE
instance to receive user data via the "send" method. Passing any
options e.g. run(0, { options }) is ignored due to workers running
...
workers automatically. The process method leaves workers waiting for the
next job after processing. Call shutdown after processing all jobs.
...
The greatest exit status is saved among workers exiting during a run.
Call this after running or processing to check if a worker exited with
a non-zero value. For more granularity, look at the on_post_exit or
...
MCE can serialized data transfers from worker processes via helper
functions do & sendto. The main MCE thread will process these in a
serial fashion. This utilizes the Storable Perl module for passing
data from a worker process to the main MCE thread. The callback
function can optionally return a reply.
...
Worker exits entirely from MCE. $id (optional) can be used to pass the
primary key or a string along with the message. Please look at the
on_post_exit and on_post_run options for callback support.
...
Worker immediately leaves the chunking loop or user func. Called from
...
Worker starts the next iteration of the chunking loop. Called from
inside foreach, forchunk, forseq, and user_func.
...
The sendto method is called by workers to serialize data to standard
output, standard error, or to end of file. The action is done by the
...
single data argument is specified and is a reference.

   $self->sendto('stdout', \@array);
   $self->sendto('stderr', \$scalar);
   $self->sendto('file:/path/to/file', \@array);

Otherwise, the reference for \@array and \$scalar is shown, not the
content. Basically, output matches the print statement.
...
A barrier sync operation in the example code means any worker must stop
at this point until all workers reach this barrier. Barrier syncing is
useful for many computer algorithms.

Barrier synchronization is supported for task 0 only. Note: Workers are
assigned task_id 0 when omitting user_tasks. All workers assigned
task_id 0 must call sync whenever barrier syncing.
...
Consider the following example. The sync operation is done inside a for
loop along with calling the do method. A stall can occur for workers
calling sync the 2nd or 3rd time while other workers are sending results
via the do or sendto methods. It requires yet another semaphore lock file
in MCE to solve this which was not done in order to keep resources low.

Therefore, please keep this in mind when mixing $self->sync with the
$self->do or $self->sendto methods inside a loop.
...
against a pool of workers. Note: The sequence option can be used if
simply wanting to loop through a sequence of numbers in parallel one
number at a time.

Below, a callback function for displaying results is used. The logic
shows how one can display results immediately while still preserving
the output order as if processing serially. The %result hash is a
temporary cache to store results for out-of-order replies.
...
Chunking reduces overhead many folds. Instead of passing a single item
from @input_data, a chunk of $chunk_size is sent to the next available
worker. The sequence option can be used as well if simply wanting to
loop through a sequence of numbers with chunking applied in parallel.
