...
This document describes MCE version 1.499
...
=head3 SELF CHUNK CID
   ...
   use MCE CONST => 1;     ## Allowed in 1.5 and later
...
This option takes an array of tasks. Each task allows for 9 sub options.

   max_workers, use_threads, user_begin, user_func, user_end, user_args,
   chunk_size, sequence, task_end

Sequence and chunk_size were added in 1.3. User_args was introduced in 1.4.
One can specify the task_end option for the manager process to run when all
workers for that role have completed processing.

Forking and threading can be inter-mixed among tasks unless running Cygwin.
...
MCE 1.5 allows chunk_size > 1. $n_seq is a reference pointing to an array of
...
will stay alive until told to shutdown or exiting of the script. Below is
shown with overriding the default for max_workers.
   ...
   ## This was not allowed before. Fixed in 1.5.
